...
public class TemporaryTask {
    ...
    private void loadEventsForCurrentUser() {
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            String query = "SELECT term FROM temporary_task WHERE user_id = ?";
    ...
    private void saveTermToDatabase(String term) {
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            String sql = "INSERT INTO temporary_task (user_id, term) VALUES (?, ?)";
    ...
    private void deleteTermFromDatabase(String term) {
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            String sql = "DELETE FROM temporary_task WHERE user_id = ? AND term = ?";
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                statement.setInt(1, Login.getCurrentUser()); // Use the static currentUser variable
                statement.setString(2, term);
                statement.executeUpdate();
            }
        } catch (SQLException e) {
