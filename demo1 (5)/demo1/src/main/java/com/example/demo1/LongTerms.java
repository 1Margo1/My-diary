...
public class LongTerms implements TermObserver {
    ...
    private void loadEventsForCurrentUser() {
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            String query = "SELECT term FROM long_terms WHERE user_id = ?";
    ...
    void handleAdd(ActionEvent event) {
        ...
        if (!termText.isEmpty()) {
            ...
            newTerm.addObserver(this);
            termList.add(newTerm);
            text.clear();

            newTerm.notifyTermAdded();
        } else {
            showAlert("Error", "Term cannot be empty");
        }
    }
    ...
    void handleDelete(ActionEvent event) {
        ...
        if (selectedTerm != null) {
            ...
            selectedTerm.notifyTermDeleted();
        } else {
            showAlert("Error", "Select a term to delete");
        }
    }
    ...
    private void saveTermToDatabase(String term) {
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            String sql = "INSERT INTO long_terms (user_id, term) VALUES (?, ?)";
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to save term to the database");
        }
    }

    private void deleteTermFromDatabase(String term) {
        try (Connection connection = DriverManager.getConnection(url, user, password)) {
            String sql = "DELETE FROM long_terms WHERE user_id = ? AND term = ?";
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                statement.setInt(1, Login.getCurrentUser());
                statement.setString(2, term);
                statement.executeUpdate();
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to delete term from the database");
        }
    }
    ...
    public void onTermAdded(Term term) {
        System.out.println("Term added: " + term.getTerm());
    }


    public void onTermDeleted(Term term) {
        System.out.println("Term deleted: " + term.getTerm());
    }
